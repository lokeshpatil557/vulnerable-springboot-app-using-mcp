/**
 * Stable error shapes used across the MCP server. Each carries a stable
 * `code` so downstream tooling (audit log, MCP response) can switch on it.
 */
...
export class ToolUnavailableError extends Error {
  ...
  constructor(public readonly tool: string, public readonly reason: string) {
    super(`Tool '${tool}' unavailable: ${reason}`);
